...
public class SentDetectorWrapper 
{
	...
	public static SentDetectorWrapper getSentDetector() throws InvalidFormatException, IOException
	{
		...
		{
			sentDetectorWrapper = new SentDetectorWrapper(new File(DatasetPreparer.coreDataPath + "data/en-sent.bin"));
		}
	...
	public edu.nyu.en.util.Span[] detectPosMonocase(String text)
	{ 
	 ...
	 * @return
	 */
	public edu.nyu.en.util.Span[] detectPos(String text)
	{
		ArrayList<edu.nyu.en.util.Span> ret = new ArrayList<edu.nyu.en.util.Span>();
		...
		return ret.toArray(new edu.nyu.en.util.Span[ret.size()]);
	}

	private void segBuffer(ArrayList<edu.nyu.en.util.Span> ret, String buffer, int offset)
	{
		Span[] spanBuffer = sentDetector.sentPosDetect(buffer);
		for(int i=0; i<spanBuffer.length; i++)
		{
			Span span = spanBuffer[i];
			edu.nyu.en.util.Span adjustedSpan = new edu.nyu.en.util.Span(span.getStart() + offset, span.getEnd() - 1 + offset);
	...
	public static void main(String[] args) throws InvalidFormatException, IOException
	{
		...
		edu.nyu.en.util.Span[] spans = SentDetectorWrapper.getSentDetector().detectPosMonocase(text);
		
		for(edu.nyu.en.util.Span span : spans)
		{
