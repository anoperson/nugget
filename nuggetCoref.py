...
def generateDataInstance(rev, dictionaries, embeddings, features, window):
    ...
    for fet in features:
        if fet == 'word': dty = 'int32'
        else:
            dty = 'float32' if numpy.array(res[fet]).ndim == 2 else 'int32'
        res[fet] = numpy.array(res[fet], dtype=dty)
    
    return res

def make_data(revs, dictionaries, embeddings, features1, features2, window, eventTypePath, negatives=100):
    ...
    res = {}
    instanceMapping,  evnIdMapping = defaultdict(dict), defaultdict(dict)
    coutPositive = 0
    for datn in revs:
        res[datn] = defaultdict(list)
        
        iid = -1
        idDocMap, evnTypeMapping, evnSubtypeMapping, evnRealisMapping, evnSentIdMapping = defaultdict(list), {}, {}, {}, {}
        binaryFeatureMap, anchorMap = {}, {}
        for doc in revs[datn]:
            for instanceId1 in range(len(revs[datn][doc]['instances'])):
                ...
                for fetType_Subtype1, realis1, eventId1 in zip(eventTypeStorer[ikey1], realisStorer[ikey1], eventIdStorer[ikey1]):
                
                    if datn == 'train' and realisDict[rev1['realis']] != realis1:
                        print 'realis1 mismatched in make data: ', datn, realisDict[rev1['realis']], ' vs ', realis1
                        exit()
                
                    fetType1, fetSubtype1 = parseTypeSubType(fetType_Subtype1, rev1, datn, typeDict, subtypeDict)
                
                    iid += 1
                    instanceMapping[datn][iid] = ists1
                    evnIdMapping[datn][iid] = ikey1 + ' ' + eventId1
                
                    evnTypeMapping[iid] = fetType1
                    evnSubtypeMapping[iid] = fetSubtype1
                    evnRealisMapping[iid] = realis1
                    evnSentIdMapping[iid] = rev1['sentenceId']
                    idDocMap[doc] += [iid]
                
                    binaryFeatureMap[iid] = rev1['binaryFeatures']
                    anchorMap[iid] = rev1['anchor']
        
        for doc in idDocMap:
            for ii in range(len(idDocMap[doc])):
                iiid = idDocMap[doc][ii]
                fetType1, fetSubtype1, realis1, sentId1 = evnTypeMapping[iiid], evnSubtypeMapping[iiid], evnRealisMapping[iiid], evnSentIdMapping[iiid]
                binaryFeatures1, anchor1 = binaryFeatureMap[iiid], anchorMap[iiid]
                eventId1 = evnIdMapping[datn][iiid].split()[-1]
                for jj in range(ii+1, len(idDocMap[doc])):
                    jjid = idDocMap[doc][jj]
                    fetType2, fetSubtype2, realis2, sentId2 = evnTypeMapping[jjid], evnSubtypeMapping[jjid], evnRealisMapping[jjid], evnSentIdMapping[jjid]
                    binaryFeatures2, anchor2 = binaryFeatureMap[jjid], anchorMap[jjid]
                    eventId2 = evnIdMapping[datn][jjid].split()[-1]
                    
                    if sentId1 == sentId2 and anchor1 == anchor2: continue
                    
                    coreferenceLabel = 0
                    corefKey = datn + ' ' + doc
                    #if datn == 'train': print corefChainStorer[corefKey], eventId1, eventId2
                    ...
                    if datn == 'train' and coreferenceLabel == 0 and random.randint(1,100) > negatives: continue
                    
                    res[datn]['idpairs'] += [[iiid, jjid]]
                    
                    res[datn]['binaryFeatures1'] += [ binaryFeatures1 ]
                    res[datn]['binaryFeatures2'] += [ binaryFeatures2 ]
                    
                    res[datn]['position1'] += [ anchor1 ]
                    res[datn]['position2'] += [ anchor2 ]
    ...
    print '--- number of positive instances: ', coutPositive
    return res, instanceMapping,  evnIdMapping, wholeLineEventTypeRealisStorer
...
def readEventTypeFile(eventTypePath):
    ...
    eventIdStorer = defaultdict(list)
...
def convertBinaryFeatures(dat, maxBiLen, bfd):
    ...
    for corpus in dat:
        for i in range(len(dat[corpus]['idpairs'])):
...
def predict(corpus, instanceMapping, batch, reModel, idx2word, oneFet1, oneFet2, features1, features2):
    evaluateCorpus = {}
    extra_data_num = -1
    nsen = corpus['idpairs'].shape[0]
    if nsen % batch > 0:
    ...
    numBatch = evaluateCorpus['idpairs'].shape[0] / batch
    predictions_corpus = numpy.array([], dtype='int32')
    probs_corpus = []
    for i in range(numBatch):
    
        zippedCorpus = buildZipData(evaluateCorpus, instanceMapping, i, batch, oneFet1, oneFet2, features1, features2)
...
def cluster(corpus, predictions, evnIdMapping):

    tclusters = defaultdict(dict)
    tidclusters = defaultdict(int)
    for idpairs, pred in zip(corpus['idpairs'], predictions):
        if pred == 0: continue
        
        id1, id2 = idpairs[0], idpairs[1]
        
        if id1 not in evnIdMapping:
            print 'cannot find id1 : ', id1 , ' in evnIdMapping'
            exit()
        if id2 not in evnIdMapping:
            print 'cannot find id2 : ', id2 , ' in evnIdMapping'
            exit()
        
        ikey1 = evnIdMapping[id1].split()
        ikey2 = evnIdMapping[id2].split()
        
        evid1, evid2 = ikey1[3], ikey2[3]
        
        ccp1 = ikey1[0]
        ccp2 = ikey2[0]
        if ccp1 != ccp2:
            print 'corpus mismatch in clustering: ', ccp1, ccp2
            exit()
        ccp = ccp1
...
def writeout(corpus, predictions, probs, evnIdMapping, wholeLineEventTypeRealisStorerCP, ofile):

    clusters = cluster(corpus, predictions, evnIdMapping)
...
def buildZipData(corpus, instanceMapping, minibatchId, batch, oneFet1, oneFet2, features1, features2):
    
    start = minibatchId*batch
    end = (minibatchId+1)*batch
    
    idpairs = corpus['idpairs'][start:end]
    for iinbatch, idpair in enumerate(idpairs):
        id1, id2 = idpair[0], idpair[1]
        insts1 = instanceMapping[id1]
        insts2 = instanceMapping[id2]
        
        for ed in features1:
            if features1[ed] >= 0:
                oneFet1[ed][iinbatch] = insts1[ed]
        for ed in features2:
            if features2[ed] >= 0:
                oneFet2[ed][iinbatch] = insts2[ed]
        
    anchor1 = corpus['position1'][start:end]
    anchor2 = corpus['position2'][start:end]
        
    zippedData = [ oneFet1[ed] for ed in oneFet1 ] + [ oneFet2[ed] for ed in oneFet2 ]
    zippedData += [ anchor1, anchor2 ]
    
    if 'coreferenceFeatures' in corpus:
        zippedData += [corpus['coreferenceFeatures'][start:end]]
            
    if 'binaryFeatures1' in corpus:
        zippedData += [corpus['binaryFeatures1'][start:end]]
        zippedData += [corpus['binaryFeatures2'][start:end]]
...
def train(dataset_path='',
    ...
    datasets, instanceMapping,  evnIdMapping, wholeLineEventTypeRealisStorer = make_data(revs, dictionaries, embeddings, features1, features2, window, eventTypePath, negatives)
    
    dimCorpus = datasets['train']
    
    vocsize = len(idx2word)
    nclasses = 2
    nsentences = len(dimCorpus['idpairs'])
    ...
    for ffin in expected_features1:
        if features1[ffin] == 1:
            features_dim1[ffin] = len(instanceMapping['train'][0][ffin][0])
    ...
    for ffin in expected_features2:
        if features2[ffin] == 1:
            features_dim2[ffin] = len(instanceMapping['train'][0][ffin][0])
        elif features2[ffin] == 0:
            features_dim2[ffin] = embeddings[ffin].shape[1]
    
    conv_winre = len(instanceMapping['train'][0]['word'])
    ...
    for corpus in datasets:
        for ed in datasets[corpus]:
            if ed == 'label' or ed == 'idpairs' or ed.startswith('position'):
                datasets[corpus][ed] = numpy.array(datasets[corpus][ed], dtype='int32')
            else:
                datasets[corpus][ed] = numpy.array(datasets[corpus][ed], dtype='float32')
    ...
    number_batch = trainCorpus['idpairs'].shape[0] / batch
    ...
    oneFet1, oneFet2 = OrderedDict(), OrderedDict()
    for ed in features1:
        if features1[ed] >= 0:
            oneFet1[ed] = numpy.zeros((batch,) + instanceMapping['train'][0][ed].shape, dtype=instanceMapping['train'][0][ed].dtype)
    for ed in features2:
        if features2[ed] >= 0:
            oneFet2[ed] = numpy.zeros((batch,) + instanceMapping['train'][0][ed].shape, dtype=instanceMapping['train'][0][ed].dtype)
    ...
    for e in xrange(nepochs):
        ...
        for minibatch_index in numpy.random.permutation(range(number_batch)):
            ...
            zippedData = buildZipData(trainCorpus, instanceMapping['train'], minibatch_index, batch, oneFet1, oneFet2, features1, features2)
        ...
        for elu in evaluatingDataset:
            _predictions[elu], _probs[elu] = predict(evaluatingDataset[elu], instanceMapping[elu], batch, reModel, idx2word, oneFet1, oneFet2, features1, features2)
            
            writeout(evaluatingDataset[elu], _predictions[elu], _probs[elu], evnIdMapping[elu], wholeLineEventTypeRealisStorer[elu], folder + '/' + elu + '.coref.pred' + str(e))
